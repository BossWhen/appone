...
import com.example.joker.appone.adapter.ReaderAdapter;
import com.example.joker.appone.demo.Reader;

import java.util.ArrayList;
import java.util.List;

public class ReaderFind extends Fragment {
    private List<Reader> readerList = new ArrayList<>();
    private MyDatabaseHelper dbHelper;
    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.reader_find,container,false);
        return view ;
    }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        Button readerFindLoad = (Button) getActivity().findViewById(R.id.reader_find_load);
        dbHelper = new MyDatabaseHelper(getContext(),"BMS.db",null,1);
        readerFindLoad.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View view) {
                ListView listView = (ListView)getActivity().findViewById(R.id.reader_find_list);
                readerList.clear();
                EditText readerFindInput = getActivity().findViewById(R.id.reader_find_input);
                ...
                ReaderAdapter adapter = new ReaderAdapter(getContext(),R.layout.reader_find_list,readerList);
